...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_GET_VIPLEVEL_AND_POINTS, "SELECT vipLevel, points FROM dc_vip WHERE id = ? AND (RealmID = -1 OR RealmID = ?)", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_UPD_VIPLEVEL_POINTS, "UPDATE dc_vip SET vipLevel = ?, points = ? WHERE id = ? AND (RealmID = ? OR RealmID = -1)", CONNECTION_ASYNC);
}
