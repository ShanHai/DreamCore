...
struct VendorItem
{
    VendorItem(uint32 _item, int32 _maxcount, uint32 _incrtime, uint32 _ExtendedCost, uint32 _points)
        : item(_item), maxcount(_maxcount), incrtime(_incrtime), ExtendedCost(_ExtendedCost), points(_points) { }
...
struct VendorItemData
{
    ...
    void AddItem(uint32 item, int32 maxcount, uint32 ptime, uint32 ExtendedCost, uint32 points = 0)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime, ExtendedCost, points));
    }
