...
void GameEventMgr::LoadFromDB()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Vendor Additions Data...");
    {
        ...
        //                                               0           1     2     3         4         5             6
        QueryResult result = WorldDatabase.Query("SELECT eventEntry, guid, item, maxcount, incrtime, ExtendedCost, points FROM game_event_npc_vendor ORDER BY guid, slot ASC");
        ...
        else
        {
            ...
            do
            {
                ...
                newEntry.points = fields[6].GetUInt32();
...
void GameEventMgr::UpdateEventNPCVendor(uint16 event_id, bool activate)
{
    ...
    {
        if (activate)
            sObjectMgr->AddVendorItem(itr->entry, itr->item, itr->maxcount, itr->incrtime, itr->ExtendedCost, itr->points, false);
        else
